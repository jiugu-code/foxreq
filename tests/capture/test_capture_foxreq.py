...
class CaptureFoxreqTests(unittest.TestCase):
    def test_runs_each_cold_sample_in_a_fresh_serial_process(self):
        ...
        with tempfile.TemporaryDirectory(
            ...
            output = Path(capture_directory) / "foxreq-cold.jsonl"
            ...
            for command, _cwd, _environment, _timeout in calls:
                self.assertEqual("1", command[command.index("--count") + 1])
                self.assertEqual("cold", command[command.index("--mode") + 1])
            records = [json.loads(line) for line in output.read_text().splitlines()]
            self.assertEqual(["cold", "cold", "cold"], [item["label"] for item in records])
            self.assertEqual(3, manifest["process_count"])
    ...
    def test_runs_one_serial_resumption_process_against_loopback(self):
        ...
        with tempfile.TemporaryDirectory(
            ...
            self.assertEqual(
                "firefox_140_esr", command[command.index("--profile") + 1]
            )
            ...
            self.assertEqual("firefox_140_esr", manifest["profile"])
            self.assertEqual(3, manifest["count"])
            self.assertEqual(1, manifest["process_count"])
